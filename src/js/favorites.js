...
const emptyMessage = document.querySelector('.FavoritesEmptyMessage');

const LS_FAV_KEY = 'favorites';
const LS_QUOTE_KEY = 'quoteData';
...
async function handleQuote() {
  ...
  const savedQuote = JSON.parse(localStorage.getItem(LS_QUOTE_KEY));
  ...
  if (savedQuote && savedQuote.date === today) {
    ...
    try {
      const newQuote = {
        author: 'Stan Smith',
        quote:
          "I've got a plan. I'm going to work out until I'm so tired I can't be sad.",
        date: today,
      };

      localStorage.setItem(LS_QUOTE_KEY, JSON.stringify(newQuote));
...
function renderFavorites() {
  ...
  const favorites = JSON.parse(localStorage.getItem(LS_FAV_KEY)) || [];

  if (favorites.length === 0) {
    favoritesList.innerHTML = '';
    if (emptyMessage) emptyMessage.classList.remove('IsHidden');
  } else {
    if (emptyMessage) emptyMessage.classList.add('IsHidden');
    favoritesList.innerHTML = createFavListMarkup(favorites);
  }
}
...
if (favoritesList) {
  favoritesList.addEventListener('click', e => {
    ...
    if (btn.classList.contains('FavTrashBtn')) {
      deleteFavorite(id);
    }

    if (btn.classList.contains('FavStartBtn')) {
      const favorites = JSON.parse(localStorage.getItem(LS_FAV_KEY)) || [];
      const exercise = favorites.find(ex => ex._id === id);
      if (exercise) {
        openModal(exercise);
      }
    }
...
function deleteFavorite(id) {
  const favorites = JSON.parse(localStorage.getItem(LS_FAV_KEY)) || [];
  const updatedFavorites = favorites.filter(ex => ex._id !== id);

  localStorage.setItem(LS_FAV_KEY, JSON.stringify(updatedFavorites));

  renderFavorites();
}

function createFavListMarkup(data) {
  return data
    .map(
      item => `
    <li class="FavCard">
      ...
      <div class="FavTop">
        <span class="FavBadge">WORKOUT</span>
         ...
         <button class="FavTrashBtn" data-id="${item._id}">
         ...
         <button class="FavStartBtn" data-id="${item._id}">
            Start
      ...
      <div class="FavTitleRow">
         ...
         <h3 class="FavName">${capitalize(item.name)}</h3>
      </div>
      <ul class="FavInfo">
         <li class="FavText">
            <span class="InfoLabel">Burned calories:</span> 
            <span class="InfoValue">${item.burnedCalories} / ${item.time} min </span> 
         </li>
         <li class="FavText">
            <span class="InfoLabel">Body part:</span> 
            <span class="InfoValue">${capitalize(item.bodyPart)}</span> 
         </li>
         <li class="FavText">
            <span class="InfoLabel">Target:</span> 
            <span class="InfoValue">${capitalize(item.target)}</span> 
         </li>
      </ul>
    </li>
  `
