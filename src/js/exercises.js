...
export function createExerciseMarkup(exercises) {
  return exercises
    .map(({ _id, name, rating, burnedCalories, time, bodyPart, target }) => {
      return `
      <li class="ExerciseItem">
        <div class="ExerciseCardTop">
          ...
          <div class="ExerciseRatingContainer">
            <span class="ExerciseRating">${Math.round(rating).toFixed(1)}</span>
            <svg class="ExerciseStarIcon" width="13" height="13">
              <use href="${iconSprite}#star"></use>
            </svg>
          </div>

          <button class="ExerciseStartBtn" data-id="${_id}">
            Start
        ...
        <div class="ExerciseCardTitle">
          ...
          <h3 class="ExerciseName">${capitalize(name)}</h3>
        </div>

        <ul class="ExerciseInfoList">
          <li class="ExerciseInfoItem">
            <span class="InfoLabel">Burned calories:</span>
            <span class="InfoValue">${burnedCalories} / ${time} min</span>
          </li>
          <li class="ExerciseInfoItem">
            <span class="InfoLabel">Body part:</span>
            <span class="InfoValue">${capitalize(bodyPart)}</span>
          </li>
          <li class="ExerciseInfoItem">
            <span class="InfoLabel">Target:</span>
            <span class="InfoValue">${capitalize(target)}</span>
          </li>
        </ul>
      </li>
      `;
