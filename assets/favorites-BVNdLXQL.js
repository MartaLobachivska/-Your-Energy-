import{openModal as C}from"./modals-UpiE7Ork.js";import{i as g}from"./icons-C9mw2FC3.js";import"./api-CA10J7K2.js";import"./vendor-D1yz-ZLW.js";const d=document.querySelector(".QuoteText"),u=document.querySelector(".QuoteAuthor"),s=document.querySelector(".FavoritesList"),o=document.querySelector(".FavoritesEmptyMessage"),n="favorites",v="quoteData";L();function L(){h(),r(),window.addEventListener("favorites-updated",r)}async function h(){const e=new Date().toDateString(),t=JSON.parse(localStorage.getItem(v));if(t&&t.date===e)f(t);else try{const a={author:"Stan Smith",quote:"I've got a plan. I'm going to work out until I'm so tired I can't be sad.",date:e};localStorage.setItem(v,JSON.stringify(a)),f(a)}catch(a){console.error("Quote fetch error",a)}}function f({author:e,quote:t}){d&&(d.textContent=t),u&&(u.textContent=e)}function r(){if(!s)return;const e=JSON.parse(localStorage.getItem(n))||[];e.length===0?(s.innerHTML="",o&&o.classList.remove("IsHidden")):(o&&o.classList.add("IsHidden"),s.innerHTML=S(e))}s&&s.addEventListener("click",e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.id;if(t.classList.contains("FavTrashBtn")&&F(a),t.classList.contains("FavStartBtn")){const c=(JSON.parse(localStorage.getItem(n))||[]).find(p=>p._id===a);c&&C(c)}});function F(e){const a=(JSON.parse(localStorage.getItem(n))||[]).filter(l=>l._id!==e);localStorage.setItem(n,JSON.stringify(a)),r()}function S(e){return e.map(t=>`
    <li class="FavCard">
      <div class="FavHeader">
      <div class="FavTop">
        <span class="FavBadge">WORKOUT</span>

         <button class="FavTrashBtn" data-id="${t._id}">
            <svg class="FavIconTrash" width="16" height="16">
              <use href="${g}#trash"></use>
            </svg>
         </button>
      </div>
         
         <button class="FavStartBtn" data-id="${t._id}">
            Start
      ...
      <div class="FavTitleRow">
         ...
         <h3 class="FavName">${i(t.name)}</h3>
      </div>
      <ul class="FavInfo">
         <li class="FavText">
            <span class="InfoLabel">Burned calories:</span> 
            <span class="InfoValue">${t.burnedCalories} / ${t.time} min </span> 
         </li>
         <li class="FavText">
            <span class="InfoLabel">Body part:</span> 
            <span class="InfoValue">${i(t.bodyPart)}</span> 
         </li>
         <li class="FavText">
            <span class="InfoLabel">Target:</span> 
            <span class="InfoValue">${i(t.target)}</span> 
         </li>
      </ul>
    </li>
  `).join("")}function i(e){return e?e.charAt(0).toUpperCase()+e.slice(1):""}
//# sourceMappingURL=favorites-BVNdLXQL.js.map
