import{getExerciseById as y,patchRating as E}from"./api-CA10J7K2.js";import{i as d}from"./icons-mdIB9KqP.js";import"./vendor-D1yz-ZLW.js";const r=document.querySelector("#exercise-modal"),v=document.querySelector(".ExercisesList");let m=null;v&&v.addEventListener("click",async t=>{const e=t.target.closest(".ExerciseStartBtn");if(e)try{const a=await y(e.dataset.id);a&&p(a)}catch(a){console.error(a)}});function p(t){m=t,r.innerHTML=$(t),r.classList.remove("is-hidden"),document.body.classList.add("no-scroll"),window.addEventListener("keydown",h),w(t)}function w(t){const e=document.querySelector(".btn-favorite");if(!e)return;const o=(JSON.parse(localStorage.getItem("favorites"))||[]).some(s=>s._id===t._id);g(e,o),e.addEventListener("click",()=>{const s=JSON.parse(localStorage.getItem("favorites"))||[],n=s.findIndex(i=>i._id===t._id);n!==-1?(s.splice(n,1),g(e,!1)):(s.push(t),g(e,!0)),localStorage.setItem("favorites",JSON.stringify(s))})}function g(t,e){e?t.innerHTML=`Remove from favorites <svg class="modal-heart" width="20" height="20">
                <use href="${d}#heart"></use>
              </svg>`:t.innerHTML=`Add to favorites <svg class="modal-heart" width="20" height="20">
                <use href="${d}#heart"></use>
              </svg>`}function f(){if(r.classList.add("is-hidden"),r.innerHTML="",document.body.classList.remove("no-scroll"),window.removeEventListener("keydown",h),window.location.pathname.includes("favorite")){const t=new Event("favorites-updated");window.dispatchEvent(t)}}function h(t){t.code==="Escape"&&f()}r&&r.addEventListener("click",t=>{(t.target===r||t.target.closest("#modal-close"))&&f()});document.addEventListener("click",t=>{t.target.classList.contains("btn-rating")&&(r.innerHTML=x(m._id),L())});function L(){let t=0;const e=document.querySelectorAll(".star-input"),a=document.querySelector("#rating-value"),o=document.getElementById("rating-form");e.forEach(s=>{s.addEventListener("click",n=>{t=Number(n.target.dataset.value),a.textContent=`${t}.0`,e.forEach(i=>i.classList.toggle("active",i.dataset.value<=t))})}),o&&(o.onsubmit=async s=>{var l,c;s.preventDefault();const{email:n,comment:i}=s.target.elements;if(t===0)return alert("Please select a rating!");try{await E(m._id,{rate:t,email:n.value.trim(),comment:i.value.trim()}),alert("Thank you for your feedback!"),p(m)}catch(u){alert(((c=(l=u.response)==null?void 0:l.data)==null?void 0:c.message)||"Error submitting rating")}})}function $(t){const{gifUrl:e,name:a,rating:o,target:s,bodyPart:n,equipment:i,popularity:l,burnedCalories:c,description:u,_id:b}=t;return`
...
//# sourceMappingURL=modals-CgwGinBx.js.map
